...
각 검색어는 한국어 단어 1~2개로 작성한다.
"[회복방식과 관련된 대상/부위/방법] + [회복방식]" 형태처럼
구체적인 단어 하나로 의미가 좁혀지게 만든다.

예를 들어 회복방식이 "스트레칭"이면 "목 스트레칭", "허리 스트레칭"처럼,
"듣기"면 "명상 호흡", "숲 소리"처럼,
"마음 정리"면 "마음챙김", "감정 정리"처럼 작성한다.

단어를 3개 이상 조합하거나 문장 형태로 만들지 않는다.
너무 넓어서 관련 없는 결과가 섞일 수 있는
"휴식", "힐링"처럼 막연한 한 단어만 쓰지 않는다.
